...
public class ProcessModelController {
	...
	@Transactional
	public void create() {
		final SubjectModel maSubject = new SubjectModelBuilder().name("Mitarbeiter").assignedGroup("MA").build();
		final SubjectModel chefSubject = new SubjectModelBuilder().name("Chef").assignedGroup("BOSS").build();
		...
		final State ma2 = new StateBuilder().subjectModel(maSubject).name("Urlaubsantrag an Chef senden")
				.functionType(StateFunctionType.SEND).build();
		final Transition t1 = new TransitionBuilder().fromState(ma1).toState(ma2).build();
		final MessageFlow mf1 = new MessageFlowBuilder().sender(maSubject).receiver(chefSubject).state(ma2).build();
		...
		final MessageFlow mf2 = new MessageFlowBuilder().sender(chefSubject).receiver(maSubject).state(c3).build();
		...
		final MessageFlow mf3 = new MessageFlowBuilder().sender(chefSubject).receiver(maSubject).state(c4).build();
		...
		final BusinessObjectModel boAntrag = new BusinessObjectModelBuilder().name("Urlaubsantrag").addToState(ma1)
				.addToState(ma2).build();
		final BusinessObjectFieldModel boAntragField = new BusinessObjectFieldModelBuilder().fieldName("Zeitraum")
				.fieldType(FieldType.STRING).businessObjectModel(boAntrag).build();
		final BusinessObjectFieldPermission bofp = new BusinessObjectFieldPermissionBuilder()
				.businessObjectFieldModel(boAntragField).state(ma1).permission(FieldPermission.READ_WRITE)
	...
	private void saveBusinessObjectFieldModels(final BusinessObjectFieldModel... businessObjectFieldModels) {
